...
//   - staging 原子构建：先写 versions/.staging-{buildID}/db，关库后 rename 为
//     正式目录 versions/{buildID}，再切 CURRENT；任何错误路径清理中间目录。
func buildV2FromCSV(rootDir string, opts BuildOptions) (Metadata, error) {
	...
	stagingDir := filepath.Join(versionsDir, stagingDirPrefix+buildID)
	stagingDBDir := filepath.Join(stagingDir, dbDirName)
	finalDir := filepath.Join(versionsDir, buildID)

	// 清理可能残留的同名 staging（上次构建 crash 遗留）。
