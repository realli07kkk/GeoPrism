...
- `query`、`providers`、`test` 以及单个 IP / CIDR 查询在 TTY 下使用增强表格渲染
...
- `geoprism <cidr>` 会直接查询 ipdb 并返回所有与该 CIDR 相交的离线网段记录；这一步忽略 `data_source_priority`，只有 ipdb 完全未命中时，才会以 CIDR 基地址回退到当前单 IP 查询逻辑
...
- 快捷查询同样支持：`geoprism example.com -j`、`geoprism 1.1.1.1 -j`、`geoprism 1.0.0.0/24 -j`
- 不支持：`ipdb`（会输出警告并忽略）
